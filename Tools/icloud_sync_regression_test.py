...
require("await recordToSave(for: recordID)" in record_batch, "CKSyncEngine send batches must await queue-safe record materialization.")
...
record_to_save = method_body(MANAGER, "private func recordToSave")
require("await episodeRecord(for: recordID)" in record_to_save, "Episode records must be materialized asynchronously for CKSyncEngine send batches.")
require("await subscriptionRecord(for: recordID)" in record_to_save, "Subscription records must be materialized asynchronously for CKSyncEngine send batches.")
episode_record = method_body(MANAGER, "private func episodeRecord")
...
require("await episodeStatePayload(forObjectHash:" in episode_record, "Episode record materialization must fetch a queue-safe payload snapshot.")
episode_payload = method_body(MANAGER, "private func episodeStatePayload")
require("databaseManager.newBackgroundContext()" in episode_payload and "await context.perform" in episode_payload, "Episode payload snapshots must be fetched on a background Core Data context queue.")
subscription_record = method_body(MANAGER, "private func subscriptionRecord")
...
require("await subscriptionPayload(forFeedURL:" in subscription_record, "Subscription record materialization must fetch a queue-safe payload snapshot.")
subscription_payload = method_body(MANAGER, "private func subscriptionPayload(forFeedURL")
require("databaseManager.newBackgroundContext()" in subscription_payload and "await context.perform" in subscription_payload, "Subscription payload snapshots must be fetched on a background Core Data context queue.")
