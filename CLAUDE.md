...
- **Default-Listen „Zuletzt gespielt" / „Neueste" (09.07.):** `default.recentlyplayed` (`orderBy=lastPlayed`) + `default.mostrecent` (`orderBy=pubDate`) wurden früher NUR bei der Migration alter Smart-Playlists angelegt → fehlten bei Neuinstallationen. `_ensureWidgetOnlyDefaultLists` (in `_migrateDatabase`, idempotent) legt sie mit „alles anzeigen"-Flags + hohem rank (100000/100001, sortieren ans Ende) an, falls sie fehlen. Sie erscheinen im „Listen"-Menü (`DMANAGER.lists`) und damit im SmartList-Widget-Picker — NICHT aus `DMANAGER.lists` ausblenden (Vorgabe „die Listen im Menüpunkt Listen definieren, was das Widget anbietet"). Sie werden nur nicht automatisch ins Hauptmenü/Seitenleiste (`MainMenuListUIDs`) gepinnt (das ist „egal" laut User). Funktionsname `_ensureWidgetOnlyDefaultLists` ist historisch — sie sind nicht widget-exklusiv.
