...
- **BGContinuedProcessingTask: Wildcard-Registrierung wird abgelehnt (Crash, bewiesen 21.07.):** `registerForTaskWithIdentifier:@"…continued.*"` liefert auf iOS 26.5.2 in JEDER Session `NO` (Diagnostics `registered=0`), obwohl der Wildcard-Eintrag im gebauten `Info.plist` steht. `submitTaskRequest:` wirft dann `NSInternalInconsistencyException` („No launch handler registered…") statt einen NSError zu liefern → App-Absturz beim Aktivieren von „Hintergrund transkribieren". Regel: Das Registrierungsergebnis MUSS gespeichert und geprüft werden (`InstacastAppDelegate.transcriptionContinuedTasksAvailable` / `newTranscriptionContinuedTaskIdentifier`); scheitert die Wildcard, wird der konkrete Identifier `…continued.session` registriert (beide Formen nie gleichzeitig — doppelte Registrierung ist eine fatale Exception), sonst Fallback auf `BGProcessingTask`. Festgepinnt: `Tools/transcription_background_regression_test.py`.
