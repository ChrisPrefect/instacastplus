...
//        sharedWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, 320, 480) configuration:config];
...
- (void) _loadWebContent
{
    ...
    [webView evaluateJavaScript:[NSString stringWithFormat:@"setContent('%@')", content] completionHandler:^(id result, NSError * _Nullable error) {
        if (error == nil) {
...
{
    WKWebView* webView = self.sharedWebView;
    [webView evaluateJavaScript:[NSString stringWithFormat:@"setContent('')"] completionHandler:^(id result, NSError * _Nullable error) {
