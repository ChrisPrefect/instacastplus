...
- (BOOL) _presentAlertControllerAnimated:(BOOL)animated completion:(void (^)(void))completion
{
    ...
    // Configure popover presentation. For action sheets, only set default sourceView
    // if the caller hasn't already configured one (preserves caller's sourceView for
    // iOS 26 Liquid Glass morph animation).
...
- (BOOL) presentAlertControllerWithTitle:(NSString*)title message:(NSString*)message button:(NSString*)buttonTitle animated:(BOOL)animated completion:(void (^)(void))completion
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:buttonTitle style:UIAlertActionStyleCancel handler:^(UIAlertAction * action) {
                                                STRONG_SELF
...
- (BOOL) presentError:(NSError*)error
{
    ...
    return [self presentAlertControllerWithTitle:error.localizedDescription message:error.localizedRecoverySuggestion button:@"OK".ls animated:YES completion:NULL];
}
@end
