...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section {
    if (self.pageMode == ICTranscriptionSettingsPageHub) {
        return [NSString stringWithFormat:@"%@\n\n%@",
                NSLocalizedString(@"Lokal läuft die Transkription auf dem iPhone, ohne Kosten. Dafür sind Modell-Downloads nötig, es kostet Rechenzeit und Akku, und InstacastPlus muss dabei laufen.\n\nServerbasiert übernimmt transcript.instacast.ch das Transkript, die Kapitel, die Sponsorerkennung und die Zusammenfassung. Das ist deutlich schneller, belastet Gerät und Akku nicht und läuft weiter, wenn InstacastPlus beendet ist.\n\nBeide Varianten dürfen gleichzeitig aktiv sein; im Episodenmenü wählst du dann pro Folge.", nil),
